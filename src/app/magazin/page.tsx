                  ...
                  <Link href={`/magazin/${featuredPost.slug}`} className="grid lg:grid-cols-[1.1fr_1fr]">
                    <div className="relative min-h-[320px] overflow-hidden border-b border-white/10 bg-black/40 lg:border-b-0 lg:border-r">
                      {featuredPost.coverImage?.url ? (
                        ...
                        <div className="flex h-full items-center justify-center text-base text-accent-300">Kein Cover</div>
                      )}
                    </div>
                    <div className="flex flex-col justify-between space-y-5 p-7 md:p-9">
                      <div className="space-y-4">
                        <div className="flex flex-wrap gap-2">
                          {(featuredPost.categories || []).slice(0, 2).map((item) => (
                            <span
                              key={item.category.name}
                              className="rounded-full border border-primary-500/30 bg-primary-500/15 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-primary-200"
                            >
                              {item.category.name}
                            </span>
                          ))}
                        </div>
                        <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-primary-300">
                        ...
                        <p className="text-base leading-relaxed text-accent-100">
                          {featuredPost.excerpt || 'Mehr lesen ...'}
                        </p>
                      </div>
                      <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-primary-300">
                    ...
                    >
                      <Link href={`/magazin/${post.slug}`} className="block h-full">
                        <div className="relative aspect-[16/10] overflow-hidden border-b border-white/10 bg-black/30">
                          ...
                          {(post.categories || [])[0] ? (
                            <span className="absolute left-4 top-4 rounded-full border border-white/20 bg-black/50 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-white backdrop-blur">
                        ...
                        <div className="space-y-4 p-6">
                          ...
                          <h3 className="font-serif text-3xl font-bold leading-tight text-white transition-colors group-hover:text-primary-200">
                            {post.title}
                          </h3>
                          <p className="line-clamp-3 text-sm leading-relaxed text-accent-200">
                            {post.excerpt || 'Mehr lesen ...'}
                          </p>
                          <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-primary-300">
                            Weiterlesen <span aria-hidden>→</span>
                          </span>
                        </div>
                      </Link>
